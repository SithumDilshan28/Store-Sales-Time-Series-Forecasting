...
def app():
  ...
  # Define your numeric and categorical columns as they were during model training
    ...
    categorical_cols = ['family', 'day_name', 'city', 'state']
     ...
     # Inputs
    store_nbr = st.sidebar.number_input('Store Number', min_value=1, max_value=54, value=1, help='Enter the store number between 1 and 54.')
    family = st.sidebar.selectbox('Product Family', options=[
        ...
        'SCHOOL AND OFFICE SUPPLIES', 'SEAFOOD'], help='Select the product family.')
    onpromotion = st.sidebar.number_input('Number of Products on Promotion', value=0, help='Specify how many products are on promotion.')
    month = st.sidebar.selectbox('Month', options=list(range(1, 13)), format_func=lambda x: f'{x:02d}', help='Select the month as a number.')
    year = st.sidebar.selectbox('Year', options=[2013, 2014, 2015, 2016, 2017], help='Choose the year of sale.')
    day_name = st.sidebar.selectbox('Day of the Week', options=[
        'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'], help='Select the day of the week.')
    city = st.sidebar.selectbox('City', options=[
        'Quito', 'Santo Domingo', 'Cayambe', 'Latacunga', 'Riobamba', 'Ibarra',
        'Guaranda', 'Puyo', 'Ambato', 'Guayaquil', 'Salinas', 'Daule', 'Babahoyo',
        'Quevedo', 'Playas', 'Libertad', 'Cuenca', 'Loja', 'Machala', 'Esmeraldas',
        'Manta', 'El Carmen'], help='Choose the city where the store is located.')
    state = st.sidebar.selectbox('State', options=[
        'Pichincha', 'Santo Domingo de los Tsachilas', 'Cotopaxi', 'Chimborazo',
        'Imbabura', 'Bolivar', 'Pastaza', 'Tungurahua', 'Guayas', 'Santa Elena',
        'Los Rios', 'Azuay', 'Loja', 'El Oro', 'Esmeraldas', 'Manabi'], help='Select the state of the store.')
